...
import type { CharDef, CharId, Mode } from '@/game/types';
...
export default function CharacterSelect({ mode, onDone, onBack }: Props) {
  const [cur, setCur] = useState<[number, number]>([0, 1]);
  const [locked, setLocked] = useState<[boolean, boolean]>([false, false]);
  const [turn, setTurn] = useState<0 | 1>(0); // who is choosing
  ...
  const isAi: [boolean, boolean] = [mode === 'cpu', mode !== '2p'];
  ...
  // CPU roulette when it's an AI's turn
  useEffect(() => {
    const side = turn;
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      switch (e.code) {
        ...
        case 'ArrowLeft':
          e.preventDefault();
          if (p2 && !lk[1] && lk[0]) move(1, -1, 0);
          else if (!p2 && !lk[0]) move(0, -1, 0);
          break;
        case 'ArrowRight':
          e.preventDefault();
          if (p2 && !lk[1] && lk[0]) move(1, 1, 0);
        ...
        case 'ArrowUp':
          e.preventDefault();
          if (p2 && !lk[1] && lk[0]) move(1, 0, -1);
          else if (!p2 && !lk[0]) move(0, 0, -1);
          break;
        case 'ArrowDown':
          e.preventDefault();
          if (p2 && !lk[1] && lk[0]) move(1, 0, 1);
  ...
  const status = locked[0] && locked[1] ? '決定！ ✝' : !locked[0] ? (isAi[0] ? 'CPUが✝本質✝に導かれて選択中…' : '1P：キャラクターを選べ') : isAi[1] ? 'CPUが✝本質✝に導かれて選択中…' : '2P：キャラクターを選べ';
    ...
    <div className="relative flex min-h-screen w-full flex-col items-center overflow-hidden bg-[#0b0b18] px-3 py-4 text-slate-100">
      ...
      <header className="relative z-10 flex w-full max-w-6xl items-center justify-between">
        ...
        <div className="text-center">
          ...
          <div className="text-xs text-slate-300 md:text-sm">生徒を選べ ── 偏差値は関係ない。地面を見ろ。</div>
        </div>
        <div className="w-20 text-right text-xs text-slate-400">{mode === '1p' ? '1P vs CPU' : mode === '2p' ? '2P 対戦' : '自己対話'}</div>
      </header>
      ...
      <div className="relative z-10 mt-4 grid w-full max-w-6xl grid-cols-1 gap-4 lg:grid-cols-[1fr_auto_1fr]">
        ...
        <div className="order-first grid grid-cols-3 gap-2 self-center lg:order-none">
          {CHAR_ORDER.map((id, i) => {
            const d = CHARS[id];
            const s1 = cur[0] === i;
            const s2 = cur[1] === i && (locked[0] || isAi[0]);
            return (
              <button
                key={id}
              ...
              >
                ...
                {s2 && <Tag text={isAi[1] ? 'CPU' : '2P'} color="bg-rose-400" side="right" locked={locked[1]} />}
              </button>
            );
          })}
        </div>
        <DetailPanel def={c2} side={1} label={isAi[1] ? 'CPU' : '2P'} locked={locked[1]} dim={!locked[0] && !isAi[0]} />
      </div>

      <div className="relative z-10 mt-4 text-center text-xs text-slate-400 md:text-sm">
        1P：WASD 移動・F 決定・G 戻る ／ 2P：矢印 移動・K 決定・L 戻る ／ クリック・タップでも選べる（同じカードをもう一度で決定）
      </div>
    </div>
  );
}
...
function DetailPanel({ def, side, label, locked, dim }: { def: CharDef; side: 0 | 1; label: string; locked: boolean; dim?: boolean }) {
  const right = side === 1;
  return (
